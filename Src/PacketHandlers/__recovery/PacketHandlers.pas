...
begin
  ...
  // Mensagem opcional de desenvolvimento - remova se não for necessária
  // Player.SendClientMessage('Dungeons estão em desenvolvimento.');

  Logger.Write('Tentando entrar na dungeon. Player.OpennedOption: ' + IntToStr(Player.OpennedOption), TLogType.ServerStatus);

  // Verificar se a opção está correta para prosseguir
  if (Player.OpennedOption = 26) then
  begin
    if (Player.PartyIndex <> 0) then
    begin // está em um grupo
      if (Player.Party.Leader <> Player.Base.ClientID) then
      begin
        Player.SendClientMessage('Você não é o líder do grupo.');
        Logger.Write('Falha: jogador não é o líder do grupo.', TLogType.ServerStatus);
        Exit;
      end;

      // Verificação para cada dungeon
      case Player.OpennedNPC of
        DUNGEON_NPC_PRISON:
          begin
            Logger.Write('Dungeon PRISON não implementada.', TLogType.ServerStatus);
          end;
        DUNGEON_NPC_MINA_1:
          begin
            LevelMin := Servers[Player.ChannelIndex].DGMines1[Packet.Dificult].LevelMin;
            for I in Player.Party.Members do
            begin
              if (LevelMin > Servers[Player.ChannelIndex].Players[I].Base.Character.Level) then
              begin
                Player.SendClientMessage('A dungeon selecionada requer nível [' +
                  AnsiString(LevelMin.ToString) + '].');
                Logger.Write('Falha: nível insuficiente para dungeon MINA_1. Player: ' + Servers[Player.ChannelIndex].Players[I].Base.Character.Name, TLogType.ServerStatus);
                Exit;
              end;
            end;
            Player.SendDungeonLobby(True, DUNGEON_LOST_MINES, Packet.Dificult);
            Logger.Write('Entrada na dungeon MINA_1 iniciada com sucesso.', TLogType.ServerStatus);
          end;
        DUNGEON_NPC_KINARY:
          begin
            LevelMin := Servers[Player.ChannelIndex].DGKinary[Packet.Dificult].LevelMin;
            for I := Low(Player.Party.Members.ToArray) to High(Player.Party.Members.ToArray) do
            begin
              if (LevelMin > Servers[Player.ChannelIndex].Players[I].Base.Character.Level) then
              begin
                Player.SendClientMessage('A dungeon selecionada requer nível [' +
                  AnsiString(LevelMin.ToString) + '].');
                Logger.Write('Falha: nível insuficiente para dungeon KINARY. Player: ' + Servers[Player.ChannelIndex].Players[I].Base.Character.Name, TLogType.ServerStatus);
                Exit;
              end;
            end;
            Player.SendDungeonLobby(True, DUNGEON_KINARY_AVIARY, Packet.Dificult);
            Logger.Write('Entrada na dungeon KINARY iniciada com sucesso.', TLogType.ServerStatus);
          end;
        else
          Logger.Write('Erro: Dungeon NPC não reconhecido. NPC ID: ' + IntToStr(Player.OpennedNPC), TLogType.Error);
      end;
    end
    else
    begin
      // Lógica para jogador solo
      case Player.OpennedNPC of
        DUNGEON_NPC_MINA_1:
          begin
            LevelMin := Servers[Player.ChannelIndex].DGMines1[Packet.Dificult].LevelMin;
            if (LevelMin > Player.Base.Character.Level) then
            begin
              Player.SendClientMessage('A dungeon selecionada requer nível [' +
                AnsiString(LevelMin.ToString) + '].');
              Logger.Write('Falha: nível insuficiente para dungeon MINA_1 solo. Player: ' + Player.Base.Character.Name, TLogType.ServerStatus);
              Exit;
            end;
            Player.SendDungeonLobby(False, DUNGEON_LOST_MINES, Packet.Dificult);
            Logger.Write('Entrada solo na dungeon MINA_1 iniciada com sucesso.', TLogType.ServerStatus);
          end;
        DUNGEON_NPC_KINARY:
          begin
            LevelMin := Servers[Player.ChannelIndex].DGKinary[Packet.Dificult].LevelMin;
            ...
            begin
              Player.SendClientMessage('A dungeon selecionada requer nível [' +
                AnsiString(LevelMin.ToString) + '].');
              Logger.Write('Falha: nível insuficiente para dungeon KINARY. Player: ' + Player.Base.Character.Name, TLogType.ServerStatus);
              Exit;
            end;
            Player.SendDungeonLobby(False, DUNGEON_KINARY_AVIARY, Packet.Dificult);
            Logger.Write('Entrada solo na dungeon KINARY iniciada com sucesso.', TLogType.ServerStatus);
          end;
        else
          Logger.Write('Erro: Dungeon NPC não reconhecido para entrada solo. NPC ID: ' + IntToStr(Player.OpennedNPC), TLogType.Error);
      end;
    end;
  end
  else
  begin
    Logger.Write('Erro: Player.OpennedOption não é 26.', TLogType.Error);
    Exit;
  end;
