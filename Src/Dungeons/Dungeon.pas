  ...
  try
    ...
    begin
      ...
      begin
        ...
        Servers[Self.Base.ChannelId].Players[Self.AttackerID].Teleport(TPosition.Create(3399, 564)); // Define posição fora da dungeon
      end
      else
      begin
        // Se for o último membro, fechar a instância da dungeon
...
begin
  ...
  begin
    ...
    begin
      DungeonInstances[Self.InstanceID].InstanceOnline := False;
      // Código para teleportar todos os jogadores para fora da dungeon e limpar os atributos relacionados
      for j := Low(DungeonInstances[Self.InstanceID].Party.Members.ToArray) to High(DungeonInstances[Self.InstanceID].Party.Members.ToArray) do
      begin
        Cid1 := DungeonInstances[Self.InstanceID].Party.Members.ToArray[j];
        if Servers[Self.ChannelId].Players[Cid1].InDungeon then
        begin
          Servers[Self.ChannelId].Players[Cid1].SendClientMessage('O grupo foi desfeito. Você será teleportado para fora da dungeon.');
          Servers[Self.ChannelId].Players[Cid1].Teleport(TPosition.Create(3399, 564)); // Teleporte para fora da dungeon
          Servers[Self.ChannelId].Players[Cid1].InDungeon := False;
          Servers[Self.ChannelId].Players[Cid1].DungeonID := 0;
          Servers[Self.ChannelId].Players[Cid1].DungeonIDDificult := 0;
          Servers[Self.ChannelId].Players[Cid1].DungeonInstanceID := 0;
        end;
      end;
      Exit; // Sair do loop principal e terminar a execução da thread
    end;
